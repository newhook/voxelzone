...
export class PlayState implements IGameState {
  ...
  private createVoxelStructures(radius: number): void {
    ...
    this.createSafeZone(20);
    
    // Create 4 distinct quadrants with different environments
    this.createForestZone(radius, 0, Math.PI/2);           // Northeast quadrant: Dense forest
    this.createUrbanZone(radius, Math.PI/2, Math.PI);      // Northwest quadrant: Urban area
    this.createDesertZone(radius, Math.PI, 3*Math.PI/2);   // Southwest quadrant: Desert with rock formations
    this.createMountainZone(radius, 3*Math.PI/2, 2*Math.PI); // Southeast quadrant: Rocky mountain area
    
    // Create connecting roadways between quadrants
    this.createRoadways(radius);
    
    // Create strategic central fortress
    createFortress(this.voxelWorld, 0, 0);
    
    // Add tactical water features
    this.createWaterFeatures(radius);
  }
  
  // Create a safe starting zone for the player
  private createSafeZone(radius: number): void {
  ...
  private createForestZone(radius: number, startAngle: number, endAngle: number): void {
    ...
    for (let patch = 0; patch < 4; patch++) {
      ...
      for (let i = 0; i < treeCount; i++) {
        ...
        if (globalAngle >= startAngle && globalAngle < endAngle) {
          ...
          if (Math.random() > 0.7) {
            ...
            createBush(this.voxelWorld, bushX, bushZ);
          }
        }
      }
      ...
      if (Math.random() > 0.5) {
        createPond(this.voxelWorld, patchCenterX, patchCenterZ);
      }
    }
  ...
  private createUrbanZone(radius: number, startAngle: number, endAngle: number): void {
        ...
        if (normalizedAngle >= startAngle && normalizedAngle < endAngle) {
          ...
          if (Math.random() > 0.4) {
            ...
            if (Math.random() > 0.5) {
              ...
              createBarrier(this.voxelWorld, barrierX, barrierZ);
            }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dz = -1; dz <= 1; dz++) {
          createBuilding(this.voxelWorld, x + dx * 5, z + dz * 5);
        }
      }
  ...
  private createDesertZone(radius: number, startAngle: number, endAngle: number): void {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (Math.random() > 0.5) {
        for (let j = 0; j < 3; j++) {
          ...
          if (normalizedAngle >= startAngle && normalizedAngle < endAngle) {
            createRockFormation(this.voxelWorld, rockX, rockZ);
          }
        }
      }
    }
    ...
    createPond(this.voxelWorld, oasisX, oasisZ);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      createTree(this.voxelWorld, treeX, treeZ);
  ...
  private createMountainZone(radius: number, startAngle: number, endAngle: number): void {
    ...
    for (let i = -ridgeLength/2; i < ridgeLength/2; i += 5) {
      ...
      if (normalizedAngle >= startAngle && normalizedAngle < endAngle) {
        ...
        // Create rock formations
        createRockFormation(this.voxelWorld, ridgeX, ridgeZ);
        ...
        for (let j = 0; j < 3; j++) {
          ...
          createRockFormation(this.voxelWorld, offsetX, offsetZ);
        }
        ...
        if (Math.random() > 0.7) {
          ...
          createPineTree(this.voxelWorld, treeX, treeZ);
        }
      }
    }
    ...
    for (let i = passIndex - passSize/2; i < passIndex + passSize/2; i += 5) {
      ...
      createBarrier(this.voxelWorld, barrier1X, barrier1Z);
      createBarrier(this.voxelWorld, barrier2X, barrier2Z);
    }
  }

  // Create roadways connecting the different quadrants
  private createRoadways(radius: number): void {
  ...
  private createWaterFeatures(radius: number): void {
    ...
    for (let i = 0; i < riverPointCount; i++) {
      ...
      createPond(this.voxelWorld, point.x, point.z);
      ...
      if (Math.random() > 0.5) {
        ...
        if (midDistanceFromOrigin > 30) {
          createPond(this.voxelWorld, middleX, middleZ);
        }
      }
      ...
      for (let j = 0; j < 3; j++) {
        ...
        if (Math.random() > 0.5) {
          createTree(this.voxelWorld, bankX, bankZ);
        } else {
          createBush(this.voxelWorld, bankX, bankZ);
        }
      }
    }
  }
