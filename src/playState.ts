...
import { worldToVoxel, voxelProperties } from './voxel';
...
export class PlayState implements IGameState {
   ...
   */
  private createBuilding(x: number, y: number, z: number): void {
    ...
    for (let i = 0; i < windowCount; i++) {
      ...
      if (windowX !== undefined && windowZ !== undefined) {
        ...
        this.voxelWorld.setVoxel({ x: windowX, y: windowY, z: windowZ }, undefined);
      }
    }
  }
  ...
  private createBarrier(x: number, y: number, z: number): void {
    ...
    switch (barrierType) {
      ...
      case 1: // Concrete barriers
        this.createConcreteBarriers(x, y, z);
        break;
      case 2: // Metal barricade
        this.createMetalBarricade(x, y, z);
        break;
      case 3: // Wooden fence
        this.createWoodenFence(x, y, z);
        break;
    }
  }
  ...
  private createSandbagWall(x: number, y: number, z: number): void {
    ...
    for (let i = 0; i < length; i++) {
      for (let h = 0; h < 2; h++) { // 2 sandbags high
        if (direction === 'x') {
          this.voxelWorld.setVoxel({ x: x + i, y: y + h + 1, z }, VoxelMaterial.SAND);
        } else {
          this.voxelWorld.setVoxel({ x, y: y + h + 1, z: z + i }, VoxelMaterial.SAND);
        }
      }
    }
  }
  ...
  private createConcreteBarriers(x: number, y: number, z: number): void {
    ...
    for (let i = 0; i < length; i++) {
      if (Math.random() > 0.2) { // 80% chance to place a barrier (creates gaps)
        if (direction === 'x') {
          this.voxelWorld.setVoxel({ x: x + i, y: y + 1, z }, VoxelMaterial.CONCRETE);
          this.voxelWorld.setVoxel({ x: x + i, y: y + 2, z }, VoxelMaterial.CONCRETE);
        } else {
          this.voxelWorld.setVoxel({ x, y: y + 1, z: z + i }, VoxelMaterial.CONCRETE);
          this.voxelWorld.setVoxel({ x, y: y + 2, z: z + i }, VoxelMaterial.CONCRETE);
        }
      }
    }
  }
  ...
  private createMetalBarricade(x: number, y: number, z: number): void {
    ...
    for (let i = 0; i < length; i++) {
      if (direction === 'x') {
        ...
        if (i % 2 === 0) {
          this.voxelWorld.setVoxel({ x: x + i, y: y + 2, z }, VoxelMaterial.METAL);
          this.voxelWorld.setVoxel({ x: x + i, y: y + 3, z }, VoxelMaterial.METAL);
        }
      } else {
        this.voxelWorld.setVoxel({ x, y: y + 1, z: z + i }, VoxelMaterial.METAL);
        // Add vertical supports every few blocks
        if (i % 2 === 0) {
          this.voxelWorld.setVoxel({ x, y: y + 2, z: z + i }, VoxelMaterial.METAL);
          this.voxelWorld.setVoxel({ x, y: y + 3, z: z + i }, VoxelMaterial.METAL);
        }
      }
    }
  }
  ...
  private createWoodenFence(x: number, y: number, z: number): void {
    ...
    for (let i = 0; i < length; i++) {
      if (direction === 'x') {
        // Fence posts
        if (i % 2 === 0) {
          this.voxelWorld.setVoxel({ x: x + i, y: y + 1, z }, VoxelMaterial.WOOD);
          this.voxelWorld.setVoxel({ x: x + i, y: y + 2, z }, VoxelMaterial.WOOD);
        }
        // Horizontal beam
        this.voxelWorld.setVoxel({ x: x + i, y: y + 2, z }, VoxelMaterial.WOOD);
      } else {
        // Fence posts
        if (i % 2 === 0) {
          this.voxelWorld.setVoxel({ x, y: y + 1, z: z + i }, VoxelMaterial.WOOD);
          this.voxelWorld.setVoxel({ x, y: y + 2, z: z + i }, VoxelMaterial.WOOD);
        }
        // Horizontal beam
        this.voxelWorld.setVoxel({ x, y: y + 2, z: z + i }, VoxelMaterial.WOOD);
      }
   ...
   */
  private createFortress(x: number, y: number, z: number): void {
